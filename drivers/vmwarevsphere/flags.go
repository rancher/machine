...
func (d *Driver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		...
		mcnflag.StringFlag{
			EnvVar: "VSPHERE_USERDATA",
			Name:   "vmwarevsphere-userdata",
			Usage:  "Filepath to a user-data.yaml file to use for ",
		},
		mcnflag.StringFlag{
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	...
	d.UserData = flags.String("vmwarevsphere-userdata")
	if d.UserData != "" {
		if _, err := os.Stat(d.UserData); err != nil {
			return err
		}
		ud, err := ioutil.ReadFile(d.UserData)
		if err != nil {
			return err
		}
