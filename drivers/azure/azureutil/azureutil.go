...
func (a AzureClient) RegisterResourceProviders(namespaces ...string) error {
	providerList, err := a.providersClient().List(context.TODO(), nil, "")
	if err != nil {
		return err
	}
	l := providerList.Response()
	if l.Value == nil {
...
func (a AzureClient) removeOSDiskBlob(resourceGroup, vmName, vhdURL string) error {
	...
	bsc, err := blobstorage.NewClient(storageAccount, storageAccountKey, blobServiceBaseURL, defaultStorageAPIVersion, true)
	if err != nil {
	...
	bs := bsc.GetBlobService()
	containerRef := bs.GetContainerReference(vhdContainer)
	if ok, err := containerRef.DeleteIfExists(nil); err != nil {
		log.Debugf("Container remove happened: %v", ok)
	}

	cts, err := bs.ListContainers(blobstorage.ListContainersParameters{})
	if err != nil {
		return err
	}
...
func (a AzureClient) CreateAvailabilitySetIfNotExists(ctx *DeploymentContext, resourceGroup, name, location string, isManaged bool, faultCount int32, updateCount int32) error {
	...
	as, err := a.availabilitySetsClient().Get(context.TODO(), resourceGroup, name)
	if err != nil {
